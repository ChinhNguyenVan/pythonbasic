...
#Kết quả: 3
# 3, max().
# Hàm này sẽ trả về phần tử có giá trị lớn nhất trong list. Nếu là chuỗi thì nó sẽ trả về phần tử có độ dài 
# chuỗi dài nhất, nếu là số thì nó sẽ trả về phần tử có số lớn nhất.
...
#Kết quả: 3
# 4, min().
# Hàm này sẽ trả về phần tử có giá trị nhỏ nhất trong list. Nếu là chuỗi thì nó sẽ trả về phần tử có độ dài 
# chuỗi ngắn nhất, nếu là số thì nó sẽ trả về phần tử có số nhỏ nhất.
...
print(list)
# Kết quả: ['A', 'B', 'C', 'D', 'E', 'F']
# Lưu ý: Ở đây mình đã cố tình để ví dụ của phương thức append() và extend() là giống nhau để cho các 
# bạn thấy được sự khác biệt giữa 2 phương thức này
...
# Kết quả: 1
# 8, index().
# Phương thức này có tác dụng trả về index xuất hiện đầu tiên của phần tử mà bạn muốn tìm và nếu như không tìm thấy 
# thì nó sẽ gọi exception.
...
# mylist là list mà các bạn cần xóa phần tử.
# index là index của phần tử mà bạn muốn muốn xóa trong list mylist. 
# Mặc định thì index = mylist[-1] (phần tử cuối cùng trong list).
# VD:
...
# key là callback def để xử lý list hoặc là một lamda function (thường được dùng để sắp xếp các list tuple hoặc 
# dictionary).
# VD: 
